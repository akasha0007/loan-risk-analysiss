...
Loaded the dataset into MySQL and wrote business queries to analyze default rate across key segments: employment type, income bracket, DTI ratio, and credit score. Full queries in [`sql/business_queries.sql`](sql/business_queries.sql).
...
Each loan is scored using engineered flags (low income, unemployment, high DTI, poor credit) and classified into **Low / Medium / High** risk. Full script in [`python/risk_scoring.py`](python/risk_scoring.py).
...
An interactive dashboard visualizes overall portfolio health, risk distribution, and segment-level default rates. See [`powerbi/`](powerbi/) for the `.pbix` file and a static preview image.
...
1. Load `data/Loan_default.csv` into MySQL using the schema in `sql/business_queries.sql`
2. Run `python/risk_scoring.py` to generate risk scores (update DB credentials first)
3. Open `powerbi/Loan_Risk_Dashboard.pbix` in Power BI Desktop, refresh the data connection
